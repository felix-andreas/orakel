...
pub async fn endpoints(env: &Env) -> String {
    let rows = vec![
        ...
        row(
            "GET /backtest/data/&lt;set&gt;/v&lt;n&gt;.json",
            "{series:[{label, color, points:[{t, v}]}]}",
            "one equity curve per policy, for one signal set and policy version (/execution/data/… 302s here)",
            "/backtest/data/ladder-rv-hist/v2.json",
        ),
        row(
            "GET /table.js",
        ...
        section(
            ...
            &table(
                ...
                &[
                    ...
                    vec!["/backtest".into(), "Backtest — our own signals: does any policy trade at all?".into()],
                    vec!["/backtest?tab=history".into(), "The historical signal set: the ranked matrix and the equity curves".into()],
                    vec!["/backtest?tab=method".into(), "How it works — locked capital, the fee model, what we refuse to conclude".into()],
                    vec!["/backtest?fees=v1".into(), "The same rows priced with NO venue fee (superseded)".into()],
                    vec!["/backtest?doc=summary".into(), "The engine's full write-up (?doc=design for the accounting rules)".into()],
                    vec!["/execution".into(), "302 → /backtest (the old name; the firm places no orders)".into()],
