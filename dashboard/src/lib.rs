...
fn page_operations() -> String {
    ...
    layout(
        ...
        &format!("{}{}{}", state_html, decisions, runs),
        BUILD_TIMESTAMP,
    )
}
...
fn page_predictions() -> String {
    ...
    let predictions_html = if rows.len() <= 1 {
        ...
        let n = rows.len() - 1;
        format!(
            "<p class=\"page-desc\" style=\"margin:0 0 .75rem\">{} prediction{}</p>{}",
            n,
            if n == 1 { "" } else { "s" },
            csv_table(&rows)
        )
    };
    body.push_str(&section("Prediction log", &predictions_html));
    ...
    body.push_str(&section("Resolutions", &resolutions_html));
    ...
    body.push_str(&section("Scores (aggregates)", &scores_html));
...
fn state_cards(state: &toml::Table) -> String {
    ...
    section("State", &format!("<div class=\"grid-cards\">{}</div>", cards))
        + &note(&format!(
