...
1. **Resolution model per leg** (gate-0 verified 251/255): window =
   ...
   (next contract becomes active 3 sessions before LTD). Touch = candle high ≥ B (↑)
   / low ≤ B (↓), "equal to or beyond".
2. **Touch probability**: driftless GBM in session time,
   ...
   candle source that resolves; σ secondary = IV anchor (DVOL/VIX/OVX; NVDA none).
3. **Predict** q_rv on active boards' legs with two-sided books; record CLOB midpoint.
   ...
   predictions; equity/crypto boards only sell-edge legs (or parked).
...
mirror (Pyth equity/WTI, Binance crypto), quotes in the fundable 3–50c zone with a
two-sided book. Onboarding = `applications/<board>.toml` (asset, resolution feed
...
ladderrv candles <data> <KEY> <from> <to>       # 1-min candles (BTCUSDT|ETHUSDT|USOILSPOT|SPY|NVDA|WTIU6)
...
Daily: pull candles for today (all keys incl. WTIU6 — expired Pyth contract feeds are
DELETED, we must archive the active month ourselves), `vol`, then `live` on active
boards; freeze data via r2data before committing manifests.
